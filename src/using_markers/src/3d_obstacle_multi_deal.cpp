...
private:
  ...
  int step_mark;
public:
  Node():step_mark(0),state(NEW),k(0),previous(0),x(0),y(0),z(0),is_path(false){}
...
public:
  ...
  int process_state(){
    ...
    if (node_x->get_k()>=99999) return -1;
  ...
  void set_obstacle(visualization_msgs::Marker & ob){
    for(int i=ob.pose.position.z-ob.scale.z/2-1;i<ob.pose.position.z+ob.scale.z/2+1;i++)
      for(int j=ob.pose.position.x-ob.scale.x/2-1;j<ob.pose.position.x+ob.scale.x/2+1;j++)
        for(int k=ob.pose.position.y-ob.scale.y/2-1;k<ob.pose.position.y+ob.scale.y/2+1;k++)
        {
...
void map_init(){
  ...
  //jay point cube structure,jay at (0,0,0)
  jay.pose.position.x=0;
  jay.pose.position.y=0;
  jay.pose.position.z=0;
  ...
  //jay_goal point cube structure,jay_goal at (9,9,9)
  jay_goal.pose.position.x=9;
  jay_goal.pose.position.y=9;
  jay_goal.pose.position.z=9;
  ...
  //stella point cube structure,stella at (0,0,9)
  stella.pose.position.x=0;
  stella.pose.position.y=0;
  stella.pose.position.z=9;
  ...
  //stella_goal point cube structure,stella_goal at (9,9,0)
  stella_goal.pose.position.x=9;
  stella_goal.pose.position.y=9;
  stella_goal.pose.position.z=0;
  ...
  //tom point cube structure,tom at (9,0,0)
  ...
  //tom_goal point cube structure,tom_goal at (0,9,9)
  ...
  tom_goal.pose.position.z=9;//goal height is 4, set z=-2 prevent the goal cube penetrate map
  ...
  obstacle2.id=8;
  obstacle2.pose.position.x=6;
  obstacle2.pose.position.y=8;
...
bool is_collision(Map & a, Map & b, Map & c, List & collision_state, Node & obstacle_temp){
  ...
  Node * temp_c=&c.nodes[c.start_x][c.start_y][c.start_z];
  i=0;
  //Get the step mark first, step mark AKA: time stamp
  while(true){
    temp_c->step_mark=i++;
    if(temp_c==&c.nodes[c.goal_x][c.goal_y][c.goal_z])
      break;
    temp_c=temp_c->get_previous();
  }
...
int main(int argc, char **argv){
  ...
  Map m_jay(10,10,10,0,0,0,9,9,9);
  Map m_tom(10,10,10,9,0,0,0,9,9);
  ...
  m_stella.set_obstacle(obstacle2);
